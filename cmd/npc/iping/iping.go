...
//
//	@Description
//	@Author  hd_0411_qxc  2025/1/9 9:29
//	@Update  hd_0411_qxc  2025/1/9 9:29
package iping
...
func (p *PingOption) Ping3(host string, ip net.IP) bool {
	...
	var size int = p.Size
	var timeout int64 = p.Timeout
	var neverstop bool = p.Nerverstop
	...
	for count > 0 || neverstop {
		...
		if err != nil || receive[ECHO_REPLY_HEAD_LEN+4] != msg[4] || receive[ECHO_REPLY_HEAD_LEN+5] != msg[5] || receive[ECHO_REPLY_HEAD_LEN+6] != msg[6] || receive[ECHO_REPLY_HEAD_LEN+7] != msg[7] || endduration >= int(timeout) || receive[ECHO_REPLY_HEAD_LEN] == 11 {
			...
			logs.Info("ping: 对 " + cname + "[" + host + "]" + " 的请求超时。")
		} else {
			if shortT == -1 {
				shortT = endduration
			...
			logs.Info("ping: 来自 " + cname + "[" + host + "]" + " 的回复: 字节=32 时间=" + strconv.Itoa(endduration) + "ms TTL=" + strconv.Itoa(ttl))
		}
		seq++
		count--
	}
...
func checkError3(err error) {
	if err != nil {
		logs.Error("ping:Fatal error: %s", err.Error())
		os.Exit(1)
	}
}
